                      ...
                      <Switch>
                        <Match when={isClickable(cells()[idx], winner())}>
                          <div
                            ...
                            onclick={() => move(idx, tictactoe().game)}>{cells()[idx]}</div>
                        </Match>
                        <Match when={!isClickable(cells()[idx], winner())}>
                          <div
                            class="w-32 min-w-32 max-w-32 h-32 min-h-32 max-w-32 border-2 border-gray-800 text-9xl text-purple-900 text-center"
                            classList={{ 'border-t-0': r == 0, 'border-b-0': r == 2, 'border-l-0': c == 0, 'border-r-0': c == 2 }}
                          >{cells()[idx]}</div>
                        </Match>
                      </Switch>
